...
addProblem(reg, name = "bbob", fun = function(job, data, dim, fid, iid) {
  ...
  list(bfun = bfun, dim = dim, lower = lower, upper = upper,
    ybest = smoof::getGlobalOptimum(bfun)$value)
})
...
addAlgorithm(reg, name = "cma_es", fun = function(job, data, instance) {
  ...
  res = cmaes::cma_es(par = x0, fn = loggedfun,
    lower = instance$lower, upper = instance$upper,
    control = ctrl
  )
...
addAlgorithm(reg, name = "libcmaesr", fun = function(job, data, instance) {
  ...
  res = libcmaesr::cmaes(x0 = x0, objective = loggedfun,
    lower = instance$lower, upper = instance$upper,
    control = ctrl, batch = FALSE
  )
  GET_RESULT(loggedfun, instance$ybest)
})
