...
namespace libcmaes
{
    ...
    :_hsig(1),_max_eigenv(0.0),_min_eigenv(0.0),_niter(0),_nevals(0),_kcand(1),_eigeniter(0),_updated_eigen(true),_run_status(0),_elapsed_time(0)
  {
    ...
    // libcmaesr patch: Eigen's dVec::Random() draws from C rand(), which CRAN
    // policy forbids and which ignores the user-supplied seed. Draw from a
    // mt19937 seeded from the run seed instead; the draw counter on the
    // parameters object makes every construction (e.g. IPOP/BIPOP restarts)
    // sample a fresh initial mean while staying reproducible for a given seed.
    std::mt19937 xrng(static_cast<std::mt19937::result_type>(p._seed + 0x9e3779b97f4a7c15ULL * ++p._xmean_rng_draws));
    std::uniform_real_distribution<double> xunif(-1.0,1.0);
    dVec xrand(p._dim);
	...
	if (p._x0min == dVec::Constant(p._dim,-std::numeric_limits<double>::max()))
	  _xmean = xrand * 4.0; // initial mean randomly sampled from -4,4 in all dimensions.
	else _xmean = p._x0min;
      }
    else
      {
	_xmean = 0.5*(xrand + dVec::Constant(p._dim,1.0)); // scale to [0,1].
