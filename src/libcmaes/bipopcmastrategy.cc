...
namespace libcmaes
{
  ...
  {
    // libcmaesr patch: seed from the run seed instead of std::random_device /
    // time() so BIPOP's randomized restart budgets are reproducible
    // (see https://github.com/CMA-ES/libcmaes/issues/257).
    _gen = std::mt19937(static_cast<std::mt19937::result_type>(parameters._seed));
